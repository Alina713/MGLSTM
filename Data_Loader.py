...
def load_traindata(train_file):
    ...
    x = data['train_list']
    y_idx = data['train_label']
    return x, y_idx

def load_traindata0(train_file):
    data = np.load(train_file, allow_pickle=True)
    data = data[:60000]
    return data
...
def TrainValueDataLoader(train_file0, train_file, poi_file, batchsize):
    def collate_fn_neg(data_tuple):
        ...
        for idx, d in enumerate(data):
            ...
            # trick
            while train_x0[neg]==data[idx] or neg == label[idx]:
                neg = np.random.randint(tra_len)
            data_neg.append(torch.LongTensor(train_x0[neg]))

        data_label = []
        for d in label:
            data_label.append(torch.LongTensor(train_x0[d]))
    ...
    train_x0 = load_traindata0(train_file0)
    neighbors = load_poi_neighbors(poi_file)
    tra_len = train_x0.shape[0]
...
def TrainDataValLoader(train_file0, train_file, batchsize):
# def TrainDataValLoader(train_file, batchsize):
    ...
    # train_x, train_y = load_traindata(train_file)
    train_x0 = load_traindata0(train_file0)
    # 使train_y的维度与train_x0一致,且值为零
    train_y0 = np.zeros(train_x0.shape[0])
    data_ = MyData(train_x0, train_y0)
    # data_ = MyData(train_x, train_y)
    dataset = DataLoader(data_, batch_size=batchsize, shuffle=True, collate_fn=collate_fn_neg)
